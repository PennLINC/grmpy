...
subs<-read.csv("/data/joy/BBL/projects/grmpyProcessing2017/structural/autoQA/n118_BBL_ScanID_Date_DatexScanID.csv")
...
###########################################################
##### Removes the Extra Columns From The Subject File #####
###########################################################

subjects <- subset(subs, select = -c(date:datexscanid))
...
write.csv(finaldemo, paste('/data/joy/BBL/projects/jirsaraieStructuralIrrit/data/JLF/n118_Demographics_',
format(Sys.Date(), format="%Y%m%d"),'.csv', sep=''), quote=F, row.names=F)

write.csv(finalari, paste('/data/joy/BBL/projects/jirsaraieStructuralIrrit/data/JLF/n118_Irritability_',
format(Sys.Date(), format="%Y%m%d"),'.csv', sep=''), quote=F, row.names=F)
